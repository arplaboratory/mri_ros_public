...
class Kimera_vio_data
{
    ...
    ros::Subscriber voxl_pointcloud;
    ...
    ros::Publisher voxl_pc_rep; //rep pc points depending if the user wants to start or stop tye map
...
public:
    ...
    visualization_msgs::Marker triangles, obs_magn_force_b_frame, triangles_old, triangles_difference, triangles_old_old;
    ...
    geometry_msgs::Point position_in_safe_dist; 
    float F_mag = 0.0;
    bool safe_flag = false;
    vector<geometry_msgs::Point> virtual_obstacles_vec;
    float theta_res_old = 0.0;
    float dx_sum_norm_old = 0.0;
    float dy_sum_norm_old = 0.0;

    //from Unity 
    bool start_mapping = false;
    bool reset_map_flag = false;
    bool with_holo = false;


    //Obstacle Force 
    float horizon = 2.0; //horizon in meters around the robot where the force is evaluated 
    float lamda = 1.0; //Exponential decay param 
    float Fs = 5.0; //Max magnitude of the Force 
    int avg_stamp = 10;
    float theta_res = 0.0;
    float F_magn = 0.0;
...
//Parameters
    ...
    bool flagSurfacePointcloudVoxblox = false;
    ...
    Kimera_vio_data()
    {
        ...
        voxblox_mesh = nh_.subscribe("/voxblox_node/mesh", 5, &Kimera_vio_data::voxblox_mesh_block_callback, this);
        voxl_pointcloud = nh_.subscribe("/cloud_in", 5, &Kimera_vio_data::realsense_pointcloud_callback, this); /// camera/depth/color/points  ///voxl3/dfs_point_cloud
        image_proc_point_cloud =  nh_.subscribe("/stereo_gray/dense_stereo/pointclud", 5, &Kimera_vio_data::image_proc_pointcloud_callback, this);
        ...
        quadrotor_sim_pose = nh_.subscribe("/quadrotor/odom", 10, &Kimera_vio_data::voxl3_mocap_pose_callback, this);
        voxblox_pc_surface = nh_.subscribe("/voxblox_node/surface_pointcloud", 1, &Kimera_vio_data::voxblox_surface_pc_callback, this);
        
        start_stop_mapping_bool = nh_.subscribe("/from_Unity/start_stop_mapping", 5, &Kimera_vio_data::from_unity_start_stop_mapping_callback, this);
        restart_mapping = nh_.subscribe("/from_Unity/restart_mapping", 5, &Kimera_vio_data::from_unity_reset_map_callback, this);
        mocap_scene_root_in_unity_world = nh_.subscribe("/from_unity/mocap_frame_in_unity_world_coo", 5, &Kimera_vio_data::mocap_scene_root_in_unity_frame_callback, this);
        holoPosition = nh_.subscribe("/hololens_position", 5, &Kimera_vio_data::holo_position_callback, this);
        virtual_obs = nh_.subscribe("/rrt/virtual_obstacles_coo", 5, &Kimera_vio_data::virtual_obs_callback, this);
        ...
        marker_pub_vertices_rviz = nh_.advertise<visualization_msgs::Marker>("kimera_mesh", 1);
        marker_pub_old_vertices_rviz = nh_.advertise<visualization_msgs::Marker>("kimera_mesh_old", 1);
        marker_pub_force_arrow_rviz = nh_.advertise<visualization_msgs::Marker>("f_obs_vector", 10);
        voxl_pc_rep = nh_.advertise<sensor_msgs::PointCloud2>("cloud_in_rep_", 1);
    ...
    double average(std::vector<float> const& v){
    if(v.empty()){
        return 0;
    }
    ...
    void voxblox_surface_pc_callback(const sensor_msgs::PointCloud2 &input)
    {
        ...
        flagSurfacePointcloudVoxblox = true;
    }
...
//   }
    ...
    void publish_force_obs_arrow_to_rviz(visualization_msgs::Marker arrow)
    {
        marker_pub_force_arrow_rviz.publish(arrow);
    }
    ...
    float average_theta_val(float theta)
    {
