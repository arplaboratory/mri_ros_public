...
from heuristics import distance_drone_steered_point
import math

class RRTBase(object):
    def __init__(self, X, Q,x_drone, obstacles, max_samples,horizon_ray, r, prc):
        """
    ...
    def get_nearest(self, tree, x):
        ...
        return next(self.nearby(tree, x, 10))
    ...
    def new_and_near_with_voxels(self, tree, q):
       ...
       # x_nearest = self.get_nearest(tree, x_rand)
        ...
        if not self.trees[0].V.count(x_new_BF) == 0:
            #print("[new_and_near] New point is not obstacle free or alredy in V")
            return None, None 
        
        #check the slope of the line between the new and the nearest.
        #if the nagle determine by the slope is bigger than 80 degrees discard the new point
   
        x_nearest_GF = rotation_from_BF_to_GF(x_nearest_BF, self.x_drone, self.X.yaw )
        x_new_GF = rotation_from_BF_to_GF(x_new_BF, self.x_drone,self.X.yaw )
     
        m_line = (x_new_GF[1] - x_nearest_GF[1])/ (x_new_GF[0] - x_nearest_GF[0])
        gamma_slope = math.atan(m_line)
        
        if (gamma_slope*180/math.pi > 80.0 or gamma_slope*180/math.pi < -80.0):
            return None, None 
    ...
    def new_and_near(self, tree, q):
       ...
       # x_rand = self.sample_random_point_in_horizon()
        ...
        #check the angular difference between the nwe rand_BF and the previous point 

        x_nearest_BF = self.get_nearest(tree, x_rand_BF)
       
        print("x_rand_BF: ", x_rand_BF)
    ...
    def connect_to_point(self, tree, x_a, x_b):
        ...
        """
         #check also here the angle between x_b and x_a to see if it is not bigger than a certian angle 
        x_nearest_GF = rotation_from_BF_to_GF(x_a, self.x_drone, self.X.yaw )
        x_new_GF = rotation_from_BF_to_GF(x_b, self.x_drone,self.X.yaw )
    ...
    def connect_to_goal(self, tree):
        ...
        print("[connect_to_goal] x_nearest: ", x_nearest)
    ...
    def reconstruct_path(self, tree, x_init, x_goal):
        ...
        while not self.trees[tree].E[current] == x_init:
            ...
            print("[reconstruct_path] current: ", current)
        path.append(x_init)
        path.reverse()
        print("[reconstruct_path] x_init: ", x_init)
        print("[reconstruct_path] path: ", path)
        return path
